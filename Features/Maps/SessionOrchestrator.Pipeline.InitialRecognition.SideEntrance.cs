...
public sealed partial class SessionOrchestrator
{
        ...
        out MapAlignmentSession usedSeed)
    {
        ...
        if (targetResolution.IsSupported)
        {
            var fingerprint = MapFeatureCacheRules.ComputeContentFingerprint(
                candidate.Map);
        ...
        // VPSG independently estimates scale. Its fixed-scale structure
        // validation is explicitly capped at the same 3 px Chamfer ceiling.
        var strictVpsgTuning =
            MapScaleSeedResolver.CreateStrictVpsgValidationTuning(
                structureTuning);
