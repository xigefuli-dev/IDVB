...
public sealed partial class MapStatusPage : UserControl
{
    ...
    private void BuildView()
    {
        ...
        content.Children.Add(CreateBindingRow(
            "保存地图缓存",
            "保存当前地图、当前楼层和当前分辨率下最近一次成功对齐的缩放值。",
            _saveMapCacheBinding,
            MapRuntimeBindingTarget.SaveMapCache));
        _allowAutomaticMapCacheToggle.Toggled +=
            AllowAutomaticMapCache_Toggled;
        content.Children.Add(_allowAutomaticMapCacheToggle);
        ...
        content.Children.Add(recognitionParametersHeader);
        ...
        var recognitionHeaderIndex =
            content.Children.IndexOf(recognitionParametersHeader);
        while (content.Children.Count > recognitionHeaderIndex + 1)
            content.Children.RemoveAt(recognitionHeaderIndex + 1);
