...
internal sealed partial class MapLearningRepository
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (manifest is null
            || manifest.ArchitectureVersion != SiameseMapNetwork.ArchitectureVersion
            || manifest.PreprocessingVersion != MapLearningPreprocessor.Version)
        {
            return false;
        }
        var directory = GetModelDirectory(version);
        var hash = await ComputeWeightsHashAsync(directory, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            await WriteJsonAsync(Path.Combine(temporaryDirectory, "manifest.json"),
        ...
        CancellationToken cancellationToken)
    {
        ...
        await WriteReferenceAsync(LastKnownGoodReferencePath, manifest.Version,
            cancellationToken);
    }
    ...
    {
        if (!await VerifyModelAsync(version, cancellationToken))
            throw new InvalidDataException("目标模型不存在、损坏或与当前架构不兼容。");
        await WriteReferenceAsync(CurrentReferencePath, version, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            var bytes = await File.ReadAllBytesAsync(Path.Combine(directory, name),
                cancellationToken);
